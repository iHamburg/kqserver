...
<?php 	
	 ...
	 <a href="<?php echo site_url('admin/coupontypes/create');?>" class="btn btn-primary">新建类型</a>
...
//			$headers = array('区域编号','区域名','类型','会员姓名','注册时间','会员管理');
			...
			foreach ($models as $key => $list) {
				$subtypecount = count($list['subTypes']);
...
//				echo "<td>".$list['realname']."</td>";
				...
				echo "<td align='center'>".anchor('admin/coupontypes/edit/'.$list['objectId'],'编辑')." | ".anchor('admin/coupontypes/delete/'.$list['objectId'],'删除')."</td>";
				echo "</tr>";
